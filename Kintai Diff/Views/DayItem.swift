...
struct DayItem: View {
    ...
    private var isError: Bool { !entry.isSame }
}

private func calculateDifferenceInMinutes(
    with firstDuration: (hours: Int, minutes: Int),
    and secondDuration: (hours: Int, minutes: Int)
) -> Int {
    abs(
        (firstDuration.hours * 60 + firstDuration.minutes)
            - (secondDuration.hours * 60 + secondDuration.minutes)
    )
}
