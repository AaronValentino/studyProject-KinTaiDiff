...
class NativeKintaiHTMLParser {
    ...
    func parse() throws -> (month: String, entries: [DayEntry]) {
        ...
        let tableBodyContent = try firstMatch(
            for: #"<tbody.*?>(.*?)</tbody>"#,
            in: html
        )

        // 3. Split the content into rows.
        let rowContents = matches(
            for: #"<tr.*?>(.*?)</tr>"#,
            in: tableBodyContent
        )
        ...
        for rowContent in rowContents {
            ...
            if let hibihouEntry = try parseColumn(
                columnContents[0],
                for: dayDate,
                type: .hibihou
            ) {
            ...
            if let kintaiEntry = try parseColumn(
                columnContents[1],
                for: dayDate,
                type: .kintai
            ) {
    ...
    private func parseColumn(
        _ columnHTML: String,
        for dayDate: Date,
        type: DayEntry.DayEntryType
    ) throws -> DayEntry? {
        // Extract time range (e.g., "<p>09:51～15:15</p>")
        guard
            let timeRangeText = try? firstMatch(
                for: #"<p>(.*?)</p>"#,
                in: columnHTML
            )
        else {
        ...
        let (startTime, endTime) = try parseTimeRange(
            from: timeRangeText,
            on: dayDate
        )
        ...
        guard
            let totalWorkDurationText = try? firstMatch(
                for: #"<b>(.*?)</b>"#,
                in: columnHTML
            )
        else {
        ...
        let totalWorkDuration = try parseTotalWorkDuration(from: totalWorkDurationText)

        // Extract details
        let details = try parseDetails(
            from: columnHTML,
            type: type
        )
    ...
    private func parseTimeRange(from text: String, on date: Date) throws -> (start: Date, end: Date) {
        ...
        guard components.count == 2,
            ...
            let endTimeString = components.last
        else {
            throw ParserError.invalidTimeRange(text)
        }

        guard
            let startDate = combine(
                date: date,
                timeString: String(startTimeString)
            ),
            let endDate = combine(
                date: date,
                timeString: String(endTimeString)
            )
        else {
    ...
    private func parseTotalWorkDuration(from text: String) throws -> Int {
    ...
    private func parseDetails(
        from columnHTML: String,
        type: DayEntry.DayEntryType
    ) throws -> String {
        switch type {
        case .hibihou:
            guard let ulContent = try? firstMatch(for: #"<ul.*?>(.*?)</ul>"#, in: columnHTML) else {
                return ""
            }

            let listItemsHTML = matches(
                for: #"<li.*?>(.*?)</li>"#,
                in: ulContent
            )

            return listItemsHTML.map { stripHTML(from: $0) }
                .joined(separator: "\n")
        case .kintai:
